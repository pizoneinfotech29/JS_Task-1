const uniqueEmails = [];
  ...
  function addUser(event) {
    ...
    if (!fname || !lname || !mno || !email) {
      alert("First name, last name, mobile number, and email are required");
      return;
    }
    ...
    if (uniquePhoneNumbers.includes(mno)) {
      alert(
        "Phone number is already exists. Please provide a unique Phone-number."
      );
      return;
    }
 ...
 // row.cells[3].innerHTML = gender;
  ...
  function validateMob(event){
    const inputEle = event.target;
    const inputValue = inputEle.value;

    const numValue = inputValue.replace(/\D/g,"");
    inputEle.value=numValue;
  }
